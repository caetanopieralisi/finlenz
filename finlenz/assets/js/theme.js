...
async function applyTheme(){
  try{
    const { data } = await supabase
      .from("app_settings")
      .select("app_name, primary_color, accent_color, logo_text, logo_url")
      .eq("id", 1)
      .maybeSingle();
    ...
    document.querySelectorAll(".brand__mark").forEach(el => {
      if (data?.logo_url) {
        el.innerHTML = `<img src="${data.logo_url}" alt="${name}" style="width:100%;height:100%;object-fit:cover;border-radius:inherit;">`;
      } else if (data?.logo_text) {
        el.textContent = data.logo_text.slice(0, 1).toUpperCase();
      }
    });
  }catch(err){
