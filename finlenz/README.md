...
> **Atualização importante:** corrigido bug de CSS que travava a navegação (o
> atributo `hidden` estava sendo sobrescrito por regras de `display`), corrigida
> uma política de segurança do banco que causava recursão infinita e quebrava
> o painel admin, e adicionada uma trigger que cria o perfil automaticamente
> pra qualquer conta nova — inclusive as criadas direto pelo painel do
> Supabase (demo e admin). **Rode o `supabase/schema.sql` de novo** no seu
> projeto (pode rodar por cima do que já existe, é seguro) e reimplante o site.
> Depois disso, promover um admin é só isto:
> ```sql
> update profiles set is_admin = true
> where id = (select id from auth.users where email = 'seuemail@exemplo.com');
> ```
> Não precisa mais inserir a linha do perfil manualmente — a trigger já cuida disso.
